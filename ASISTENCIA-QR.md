...
- **Tomar foto del QR** permite usar la captura de imagen del teléfono o seleccionar una foto, según el navegador. La imagen se decodifica localmente y un QR válido registra la asistencia automáticamente. La guía de permisos permite copiar un enlace limpio, sin sesión ni token QR, para abrir el escáner en otro navegador.
...
- Registrar: reconocer el QR desde la cámara integrada, una foto o un enlace QR registra automáticamente cuando el operario tiene sesión. Si necesita ingresar o crear el PIN, el registro se envía al terminar ese acceso.
- Solo tras confirmación del servidor aparece la ventana **Registrado** y suena una campana. El audio depende del volumen y los permisos de reproducción del dispositivo. Las asistencias repetidas muestran **Ya registrada** sin campana; los errores muestran **Reintentar registro**, sin anunciar éxito.
